...
def generateNewMoleculeWithProperOrder(molecule, atomsIndexLstWithRightOrder):
    ...
    newIndexToOriginalIndex = {i: item for i, item in enumerate(atomsIndexLstWithRightOrder)}

    # Original code used MolFromPDBBlock(ATOM-only): no CONECT, so RDKit infers bonds from
    # distances — wrong for large fused aromatics → "ERROR in atom order!!!".
    # RenumberAtoms keeps the exact bond topology and coordinates.
    newMolecule = rdmolops.RenumberAtoms(Chem.Mol(molecule), list(atomsIndexLstWithRightOrder))
